...
exports.detectRequestedDishFamily = detectRequestedDishFamily;
exports.deriveIdeaTitleFromConversationContext = deriveIdeaTitleFromConversationContext;
exports.recipeMatchesRequestedDirection = recipeMatchesRequestedDirection;
...
function detectRequestedDishFamily(context) {
...
function detectRequestedProtein(context) {
    const normalized = normalizeText(context);
...
function detectRequestedAnchorIngredient(context) {
    const normalized = normalizeText(context);
    const anchors = [
...
function deriveIdeaTitleFromConversationContext(context) {
    const normalized = normalizeText(context);
    ...
    if (family === "pizza") {
        if (normalized.includes("focaccia"))
            return "Focaccia Pizza";
        if (protein)
            return `${protein.charAt(0).toUpperCase() + protein.slice(1)} Pizza`;
        return "Pizza";
    }
    if (family === "pasta") {
        if (anchor === "eggplant" || anchor === "aubergine" || anchor === "vinete")
            return "Eggplant Pasta";
        if (protein)
            return `${protein.charAt(0).toUpperCase() + protein.slice(1)} Pasta`;
        return "Pasta";
    }
    if (family === "skillet") {
        if (protein)
            return `${protein.charAt(0).toUpperCase() + protein.slice(1)} Skillet`;
        return "Skillet Dinner";
    }
    if (family === "bowl") {
        if (protein)
            return `${protein.charAt(0).toUpperCase() + protein.slice(1)} Bowl`;
        return "Rice Bowl";
    }
    if (family === "salad") {
        if (protein)
            return `${protein.charAt(0).toUpperCase() + protein.slice(1)} Salad`;
        return "Salad";
    }
    if (family === "tacos") {
        if (protein)
            return `${protein.charAt(0).toUpperCase() + protein.slice(1)} Tacos`;
        return "Tacos";
    }
    if (family === "soup") {
        if (protein)
            return `${protein.charAt(0).toUpperCase() + protein.slice(1)} Soup`;
        return "Soup";
    }
    if (family === "dip") {
        if (anchor === "eggplant" || anchor === "aubergine" || anchor === "vinete")
...
function recipeMatchesRequestedDirection(recipe, context) {
    const normalizedContext = normalizeText(context);
    const recipeText = normalizeText(`${recipe.title} ${recipe.description ?? ""} ${recipe.ingredients.map((item) => item.name).join(" ")} ${recipe.steps.map((item) => item.text).join(" ")}`);
