...
// Picked once at module load time (client only — this is a "use client" component)
const SELECTED_TAGLINE =
  TAGLINES[Math.floor(Math.random() * TAGLINES.length)] ?? TAGLINES[0];

export function RandomHeroTagline() {
  return (
    <div className="space-y-4" data-tagline-id={SELECTED_TAGLINE.id}>
      <h1
        className="max-w-[11ch] font-display text-[34px] font-semibold leading-[0.9] tracking-[-0.04em] text-[color:var(--landing-ink)] sm:text-[54px] lg:text-[92px]"
        suppressHydrationWarning
      >
        {SELECTED_TAGLINE.copy}
      </h1>
