import { detectRequestedDishFamily } from "../homeRecipeAlignment";

export type SeedRecipeEvalCase = {
  id: string;
  label: string;
  tier?: "baseline" | "exploratory";
  prompt: string;
  conversation: string;
  expected: {
    dishFamily: string | null;
    normalizedNameHint: string | null;
  };
};

export const SEED_RECIPE_EVAL_CASES: SeedRecipeEvalCase[] = [
  {
    id: "focaccia-pizza",
...
export function buildSeedEvalContext(testCase: SeedRecipeEvalCase) {
  return `${testCase.prompt}\n${testCase.conversation}`.trim();
}

export function evaluateSeedDishFamily(testCase: SeedRecipeEvalCase) {
  return detectRequestedDishFamily(buildSeedEvalContext(testCase));
}
