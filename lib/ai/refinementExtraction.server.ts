...
function normalizeAiRefinement(parsed: unknown, fallback: LockedDirectionRefinement): LockedDirectionRefinement | null {
  ...
  const normalized: LockedDirectionRefinement = {
    ...
    extracted_changes: {
      required_ingredients: unique(Array.isArray((extracted as { required_ingredients?: unknown[] } | undefined)?.required_ingredients) ? ((extracted as { required_ingredients?: unknown[] }).required_ingredients ?? []).filter((value): value is string => typeof value === "string") : []),
      preferred_ingredients: unique(Array.isArray((extracted as { preferred_ingredients?: unknown[] } | undefined)?.preferred_ingredients) ? ((extracted as { preferred_ingredients?: unknown[] }).preferred_ingredients ?? []).filter((value): value is string => typeof value === "string") : []),
      forbidden_ingredients: unique(Array.isArray((extracted as { forbidden_ingredients?: unknown[] } | undefined)?.forbidden_ingredients) ? ((extracted as { forbidden_ingredients?: unknown[] }).forbidden_ingredients ?? []).filter((value): value is string => typeof value === "string") : []),
