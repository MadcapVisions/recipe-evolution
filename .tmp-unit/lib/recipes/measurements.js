...
exports.normalizeMeasurementUnit = normalizeMeasurementUnit;
exports.combineMeasuredQuantities = combineMeasuredQuantities;
const UNIT_TO_BASE = {
...
function combineMeasuredQuantities(left, right) {
    ...
    if (leftMeta.family === "volume") {
        if (baseQuantity >= 48)
            return { quantity: Math.round((baseQuantity / 48) * 100) / 100, unit: "cup" };
        if (baseQuantity >= 3)
            return { quantity: Math.round((baseQuantity / 3) * 100) / 100, unit: "tbsp" };
        return { quantity: Math.round(baseQuantity * 100) / 100, unit: "tsp" };
    }
    if (leftMeta.family === "weight") {
        if (baseQuantity >= 16)
            return { quantity: Math.round((baseQuantity / 16) * 100) / 100, unit: "lb" };
